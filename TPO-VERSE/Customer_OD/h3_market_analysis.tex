...
\raggedbottom
\sloppy
% Generous float budgets: the case study is figure-dense, and the defaults
% defer everything to the end of the document.
...
\hypersetup{
  ...
  citecolor=spineTeal,
  urlcolor=spineTeal,
  pdftitle={Paragon TRITIUM: The H3 Enterprise Spine --- A Blueprint for
    Global Expansion, Every Store, Every Consumer, Every Rupiah on One
    Hexagonal Grid},
...
\twocolumn[{%
  \begin{center}
    ...
    \vspace{0.35em}
    {\large A Blueprint for Global Expansion --- Every Store, Every
      Consumer, Every Rupiah on One Hexagonal Grid\par}
    \vspace{1.1em}
    ...
    \begin{minipage}{0.86\textwidth}
      \small
      \textbf{Abstract.}
      Where should the next store open? Which consumers does the network
      already reach? Where is revenue leaking to unserved demand? Every one
      of these decisions is keyed to a map --- and in Indonesia that map is
      usually administrative geography: polygons that differ in area by
      three orders of magnitude, cut catchments at political borders, and
      are periodically redrawn (\emph{pemekaran}), breaking every
      year-over-year comparison. Paragon TRITIUM replaces them with an
      \emph{H3 Enterprise Spine}: every store, resident, consumer, and
      rupiah of revenue indexed to the same fixed hexagonal grid at
      ingestion, turning spatial analysis into ordinary SQL joins on the
      data warehouse the company already runs. On a reproducible
      Jakarta--Tangerang corridor, the spine isolates unserved
      \emph{whitespace} worth ${\sim}$IDR~8.4\,bn per month and reaches
      50\% of the population with 25\% of the land area, where district
      targeting needs 38\% --- leaner media spend, sharper expansion
      shortlists, measurable delivery SLAs. And because the grid covers
      the planet, the same schema, SQL, and playbook deploy unchanged in
      every new market: a working blueprint for global expansion, delivered
      in three phases with no new infrastructure.
      \par\vspace{0.5em}
...
\begin{figure}[!tp]
  \centering
  \input{figures/fig-hierarchy.tex}
...
\begin{figure}[!tp]
  \centering
  \resizebox{\columnwidth}{!}{\input{figures/fig-kring.tex}}
...
\begin{figure*}[!tp]
  \centering
  \resizebox{0.86\textwidth}{!}{\input{figures/fig-overlay.tex}}
...
\begin{figure*}[!tp]
  \centering
  \resizebox{0.86\textwidth}{!}{\input{figures/fig-classification.tex}}
...
\begin{figure}[!tp]
  \centering
  \resizebox{\columnwidth}{!}{\input{figures/fig-scatter.tex}}
...
\begin{figure}[!tp]
  \centering
  \resizebox{\columnwidth}{!}{\input{figures/fig-gain.tex}}
...
\begin{figure}[!tp]
  \centering
  \resizebox{0.94\columnwidth}{!}{\input{figures/fig-bars.tex}}
