...
def legend_swatches(x0, y0, items, boxw=0.42, boxh=0.26, dx=1.72):
...
def fig_classification(s=0.30):
    ...
    L.append(legend_swatches(0.1, -0.85, items, dx=2.62))
    L.append("\\end{tikzpicture}")
...
\begin{axis}[
  ...
  legend style={draw=none, fill=none, font=\small, at={(0.97,0.32)}, anchor=east},
]
...
\addplot[gridc, line width=0.7pt, forget plot] coordinates {(%(H50).1f,0) (%(H50).1f,50)};
\addplot[gridc, line width=0.7pt, forget plot] coordinates {(%(A50c).1f,0) (%(A50c).1f,50)};
\node[font=\scriptsize, text=inkS, anchor=south west] at (axis cs:%(H50).1f,51)
  {50\%% of people in %(H50).0f\%% of area};
\node[font=\scriptsize, text=inkS, anchor=north west, align=left] at (axis cs:%(A50c).1f,49)
  {districts need\\ %(A50c).0f\%% of area};
\end{axis}
\end{tikzpicture}
...
\addplot[gridc, line width=0.9pt, forget plot] coordinates {(%(PH).2f,-0.12) (%(PH).2f,3.4)};
